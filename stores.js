...
export function memoryStore() {
  ...
  return {
    ...
    put: async (id, bytes) => { blocks.set(id, bytes.slice()) },
...
export async function browserStore(name = 'andfs-v1') {
  ...
  const transaction = (table, mode, operation) => new Promise((resolve, reject) => {
  ...
  return {
    ...
    put: (id, bytes) => transaction('blocks', 'readwrite', store => store.put(bytes, id)),
    pin: id => transaction('pins', 'readwrite', store => store.put(true, id)),
...
export async function diskStore(directory) {
  ...
  return {
    async get(id) {
      try { return new Uint8Array(await fs.readFile(file(id))) }
      catch (error) { if (error.code === 'ENOENT') return undefined; throw error }
    },
    async put(id, bytes) {
      const target = file(id)
      const temporary = target + '.' + crypto.randomUUID()
      try { await fs.writeFile(temporary, bytes); await fs.rename(temporary, target) }
